...
/// 이중 과제 게임 - JSON 기반 버전
/// 
/// 두 가지 조건을 동시에 만족하는 것을 찾습니다.
class DualTaskGameV2 extends StatefulWidget {
  final String childId;
  final void Function(bool isCorrect, int responseTimeMs) onAnswer;
  final VoidCallback? onComplete;
  final int difficultyLevel;

  const DualTaskGameV2({
    super.key,
  ...
  @override
  State<DualTaskGameV2> createState() => _DualTaskGameV2State();
}

class _DualTaskGameV2State extends State<DualTaskGameV2> {
  ...
  Future<void> _loadQuestions() async {
    try {
      final content = await _loaderService.loadFromLocalJson('dual_task.json');
            ...
            Container(
              ...
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  colors: [Colors.deepOrange.shade100, Colors.pink.shade100],
                ),
                borderRadius: BorderRadius.circular(16),
              ),
              child: Column(
                children: [
                  const Text('🎯 두 가지 조건을 찾으세요!', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 12),
                  Text(currentItem.question, style: const TextStyle(fontSize: 26, fontWeight: FontWeight.bold)),
                ],
              ),
            ),
