...
/// 목표 찾기 게임 - JSON 기반 버전
class TargetHuntGameV2 extends StatefulWidget {
  final String childId;
  final void Function(bool isCorrect, int responseTimeMs) onAnswer;
  final VoidCallback? onComplete;
  final int difficultyLevel;

  const TargetHuntGameV2({
    super.key,
  ...
  @override
  State<TargetHuntGameV2> createState() => _TargetHuntGameV2State();
}

class _TargetHuntGameV2State extends State<TargetHuntGameV2> {
  ...
  Future<void> _loadQuestions() async {
    try {
      final content = await _loaderService.loadFromLocalJson('target_hunt.json');
  ...
  void _onItemTap(bool isTarget) {
    ...
    widget.onAnswer(isTarget, responseTime);
        ...
        Column(
          children: [
            ...
            Container(
              ...
              child: Column(
                children: [
                  const Text('🔍 찾아보세요!', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 12),
                  Text(currentItem.question, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
                ],
              ),
            ),
            ...
            Expanded(
              child: GridView.builder(
                ...
                itemBuilder: (context, index) {
                  final isTarget = index == 12; // 중앙에 타겟 배치
                  final emoji = isTarget ? target : ['⭐', '❤️', '🌟', '💫', '✨'][index % 5];
                  
                  return GestureDetector(
                    onTap: () => _onItemTap(isTarget),
                    child: Container(
                      decoration: BoxDecoration(
                        color: Colors.white,
