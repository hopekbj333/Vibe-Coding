...
class SelectiveAttentionGameV2 extends StatefulWidget {
  final String childId;
  final void Function(bool isCorrect, int responseTimeMs) onAnswer;
  final VoidCallback? onComplete;
  final int difficultyLevel;
  const SelectiveAttentionGameV2({super.key, required this.childId, required this.onAnswer, this.onComplete, this.difficultyLevel = 1});
  @override
  State<SelectiveAttentionGameV2> createState() => _SelectiveAttentionGameV2State();
}

class _SelectiveAttentionGameV2State extends State<SelectiveAttentionGameV2> {
  ...
  Future<void> _loadQuestions() async {
    try {
      final content = await _loaderService.loadFromLocalJson('selective_attention.json');
  ...
  Widget build(BuildContext context) {
    ...
    final currentItem = _content!.items[_currentQuestionIndex];
    return Stack(children: [Column(children: [Padding(padding: const EdgeInsets.all(16), child: _buildProgressIndicator()), const SizedBox(height: 24), Container(margin: const EdgeInsets.all(16), padding: const EdgeInsets.all(24), decoration: BoxDecoration(gradient: LinearGradient(colors: [Colors.blue.shade100, Colors.indigo.shade100]), borderRadius: BorderRadius.circular(16)), child: Column(children: [const Text('🎯 선택적 주의!', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)), const SizedBox(height: 12), Text(currentItem.question, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold))])), const SizedBox(height: 40), const Text('🔴 🔴 🔵 ⚪ 🔵', style: TextStyle(fontSize: 48), textAlign: TextAlign.center), const SizedBox(height: 40), Expanded(child: GridView.builder(padding: const EdgeInsets.all(16), gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 3, mainAxisSpacing: 16, crossAxisSpacing: 16, childAspectRatio: 1.5), itemCount: currentItem.options.length, itemBuilder: (context, index) => _buildOptionCard(index)))]), if (_answered && _isCorrect != null) FeedbackWidget(type: _isCorrect! ? FeedbackType.correct : FeedbackType.incorrect, message: _isCorrect! ? currentItem.explanation ?? FeedbackMessages.getRandomCorrectMessage() : FeedbackMessages.getRandomIncorrectMessage())]);
  }
  ...
  Widget _buildOptionCard(int index) {
    ...
    if (isSelected && _answered) { borderColor = _isCorrect! ? DesignSystem.semanticSuccess : DesignSystem.semanticError; backgroundColor = _isCorrect! ? Colors.green.shade50 : Colors.red.shade50; } else if (isSelected) { backgroundColor = Colors.indigo.shade50; borderColor = Colors.indigo; }
    return GestureDetector(onTap: () => _onOptionSelected(index), child: Container(decoration: BoxDecoration(color: backgroundColor, borderRadius: BorderRadius.circular(12), border: Border.all(color: borderColor, width: isSelected ? 3 : 2)), child: Center(child: Text(option.label, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold)))));
  }
