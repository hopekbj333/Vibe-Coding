...
/// 스트룹 과제 게임 - JSON 기반 버전
class StroopGameV2 extends StatefulWidget {
  ...
  @override
  State<StroopGameV2> createState() => _StroopGameV2State();
}

class _StroopGameV2State extends State<StroopGameV2> {
  ...
  Future<void> _loadQuestions() async {
    try {
      final content = await _loaderService.loadFromLocalJson('stroop.json');
  ...
  Widget build(BuildContext context) {
    ...
    final currentItem = _content!.items[_currentQuestionIndex];
    final word = currentItem.itemData?['word'] as String? ?? '빨강';
    final color = currentItem.itemData?['color'] as String? ?? 'red';

    // 색상 매핑
    Color textColor = Colors.red;
    if (color == 'blue') textColor = Colors.blue;
    if (color == 'green') textColor = Colors.green;
        ...
        Column(
          children: [
            ...
            Container(
              ...
              child: Column(
                children: [
                  const Text('🎨 글자의 색을 말하세요!', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            ...
            Container(
              ...
              child: Text(word, style: TextStyle(fontSize: 64, fontWeight: FontWeight.bold, color: textColor)),
            ),
