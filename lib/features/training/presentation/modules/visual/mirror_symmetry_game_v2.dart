...
/// 거울 대칭 게임 (V-03) - JSON 기반 버전
class MirrorSymmetryGameV2 extends StatefulWidget {
  final String childId;
  final void Function(bool isCorrect, int responseTimeMs) onAnswer;
  final VoidCallback? onComplete;
  final int difficultyLevel;

  const MirrorSymmetryGameV2({
    super.key,
  ...
  @override
  State<MirrorSymmetryGameV2> createState() => _MirrorSymmetryGameV2State();
}

class _MirrorSymmetryGameV2State extends State<MirrorSymmetryGameV2> {
  ...
  Future<void> _loadQuestions() async {
    try {
      final content = await _loaderService.loadFromLocalJson('mirror_symmetry.json');
  ...
  Widget build(BuildContext context) {
    ...
    final currentItem = _content!.items[_currentQuestionIndex];
    final type = currentItem.itemData?['type'] as String? ?? 'horizontal';
        ...
        Column(
          children: [
            ...
            Container(
              ...
              child: Column(
                children: [
                  Text(type == 'horizontal' ? '🪞 거울에 비친 모습은?' : '💧 물에 비친 모습은?', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
  ...
  Widget _buildOptionCard(int index) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Center(
          child: Text(option.label, style: const TextStyle(fontSize: 50, fontWeight: FontWeight.bold)),
        ),
      ),
    );
  }
