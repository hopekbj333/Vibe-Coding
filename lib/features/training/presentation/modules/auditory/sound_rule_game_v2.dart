...
/// 소리 규칙 찾기 게임 (A-05) - JSON 기반 버전
class SoundRuleGameV2 extends StatefulWidget {
  final String childId;
  final void Function(bool isCorrect, int responseTimeMs) onAnswer;
  final VoidCallback? onComplete;
  final int difficultyLevel;

  const SoundRuleGameV2({
    super.key,
  ...
  @override
  State<SoundRuleGameV2> createState() => _SoundRuleGameV2State();
}

class _SoundRuleGameV2State extends State<SoundRuleGameV2> {
  ...
  Future<void> _loadQuestions() async {
    try {
      final content = await _loaderService.loadFromLocalJson('sound_rule.json');
        ...
        Column(
          children: [
            ...
            Container(
              ...
              child: Column(
                children: [
                  const Text('🔍 패턴을 찾아보세요!', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
  ...
  Widget _buildOptionCard(int index) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Center(
          child: Text(option.label, style: const TextStyle(fontSize: 40, fontWeight: FontWeight.bold)),
        ),
      ),
    );
  }
