...
/// 환경음 식별 게임 (A-10) - JSON 기반 버전
class EnvironmentalSoundGameV2 extends StatefulWidget {
  final String childId;
  final void Function(bool isCorrect, int responseTimeMs) onAnswer;
  final VoidCallback? onComplete;
  final int difficultyLevel;

  const EnvironmentalSoundGameV2({
    super.key,
  ...
  @override
  State<EnvironmentalSoundGameV2> createState() => _EnvironmentalSoundGameV2State();
}

class _EnvironmentalSoundGameV2State extends State<EnvironmentalSoundGameV2> {
  ...
  Future<void> _loadQuestions() async {
    try {
      final content = await _loaderService.loadFromLocalJson('environmental_sound.json');
        ...
        Column(
          children: [
            ...
            Container(
              ...
              child: Column(
                children: [
                  const Text('🌍 무슨 소리일까요?', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
  ...
  Widget _buildOptionCard(int index) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Center(
          child: Text(option.label, style: const TextStyle(fontSize: 36)),
        ),
      ),
    );
  }
