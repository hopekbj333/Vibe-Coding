...
/// 리듬 패턴 게임 (A-03) - JSON 기반 버전
class RhythmPatternGameV2 extends StatefulWidget {
  final String childId;
  final void Function(bool isCorrect, int responseTimeMs) onAnswer;
  final VoidCallback? onComplete;
  final int difficultyLevel;

  const RhythmPatternGameV2({
    super.key,
  ...
  @override
  State<RhythmPatternGameV2> createState() => _RhythmPatternGameV2State();
}

class _RhythmPatternGameV2State extends State<RhythmPatternGameV2> {
  ...
  Future<void> _loadQuestions() async {
    try {
      final content = await _loaderService.loadFromLocalJson('rhythm_pattern.json');
      setState(() {
        _content = content;
        _isLoading = false;
        ...
        Column(
          children: [
            ...
            Container(
              ...
              child: Column(
                children: [
                  const Text('🎵 리듬 패턴 찾기!', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
  ...
  Widget _buildOptionCard(int index) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Center(
          child: Text(option.label, style: const TextStyle(fontSize: 36, fontWeight: FontWeight.bold)),
        ),
      ),
    );
  }
