...
/// 음높이 구별 게임 (A-07) - JSON 기반 버전
class PitchDiscriminationGameV2 extends StatefulWidget {
  final String childId;
  final void Function(bool isCorrect, int responseTimeMs) onAnswer;
  final VoidCallback? onComplete;
  final int difficultyLevel;

  const PitchDiscriminationGameV2({super.key, required this.childId, required this.onAnswer, this.onComplete, this.difficultyLevel = 1});

  @override
  State<PitchDiscriminationGameV2> createState() => _PitchDiscriminationGameV2State();
}

class _PitchDiscriminationGameV2State extends State<PitchDiscriminationGameV2> {
  ...
  Future<void> _loadQuestions() async {
    try {
      final content = await _loaderService.loadFromLocalJson('pitch_discrimination.json');
        ...
        Column(
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(gradient: LinearGradient(colors: [Colors.purple.shade100, Colors.pink.shade100]), borderRadius: BorderRadius.circular(16)),
              child: Column(children: [const Text('🎵 음높이를 구별해보세요!', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)), const SizedBox(height: 12), Text(currentItem.question, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold))]),
            ),
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) => Padding(padding: const EdgeInsets.only(bottom: 12), child: _buildOptionCard(index)),
              ),
            ),
          ],
        ),
  ...
  Widget _buildOptionCard(int index) {
    ...
    return GestureDetector(onTap: () => _onOptionSelected(index), child: Container(padding: const EdgeInsets.symmetric(vertical: 24), decoration: BoxDecoration(color: backgroundColor, borderRadius: BorderRadius.circular(16), border: Border.all(color: borderColor, width: isSelected ? 4 : 2), boxShadow: [BoxShadow(color: Colors.black12, blurRadius: 8, offset: const Offset(0, 4))]), child: Center(child: Text(option.label, style: const TextStyle(fontSize: 32, fontWeight: FontWeight.bold)))));
  }
