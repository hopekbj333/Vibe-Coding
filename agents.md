...
*   **한줄 소개**: 글을 배우기 전에 필요한 기초 인지 능력(Readiness)을 점검하는 디지털 검사 및 훈련 서비스.
...
*   **진행 중**: **Milestone 2 - 검사 기능 구현 (Assessment Feature)** 시작.
    *   사용자가 아동을 선택하고 검사 화면으로 진입하는 흐름 구현 필요.
    *   실제 검사 문항(음운 인식 등) UI 및 로직 구현 필요.
